...
import {
  Plus, Edit, Trash2, Search, MessageSquare, Send, X, Clock, CheckCircle, XCircle
} from "lucide-react";
...
export default function SMSNotificationsPage() {
  ...
  const handleEditNotification = (notification: SMSNotification) => {
  ...
  const handleDeleteNotification = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/sms-notifications?id=${id}`, {
        method: "DELETE",
      });
    ...
    try {
      ...
      if (editingNotification) {
        const response = await fetch("/api/sms-notifications", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ id: editingNotification.id, ...notificationData }),
        });
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-primary text-white py-8">
        <div className="container mx-auto px-4">
          <h1 className="font-heading text-3xl font-bold mb-2">SMS Notifications</h1>
          <p className="text-gray-200">Send SMS notifications to students, parents, and staff</p>
        </div>
      </div>
      <div className="container mx-auto px-4 py-8">
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          <div className="p-6 border-b flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <h2 className="font-heading text-xl font-bold text-primary">Notifications</h2>
                  ...
                  {filteredNotifications.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900">{item.message.substring(0, 80)}...</div>
                      </td>
