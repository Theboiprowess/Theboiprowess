...
import {
  Plus, Edit, Trash2, Search, FileText, Eye, EyeOff, X, Calendar, AlertTriangle
} from "lucide-react";
    ...
    try {
      const response = await fetch("/api/notices", {
        ...
        body: JSON.stringify({
          id: notice.id,
          published: !notice.published,
          published_at: !notice.published ? new Date().toISOString() : null,
        }),
      });
    ...
    try {
      ...
      if (editingNotice) {
        const response = await fetch("/api/notices", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ id: editingNotice.id, ...noticeData }),
        });
        if (!response.ok) throw new Error("Failed to update notice");
      } else {
        const response = await fetch("/api/notices", {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-primary text-white py-8">
        <div className="container mx-auto px-4">
          <h1 className="font-heading text-3xl font-bold mb-2">Manage Notices</h1>
          <p className="text-gray-200">Publish school notices and announcements</p>
        </div>
      </div>
      <div className="container mx-auto px-4 py-8">
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          <div className="p-6 border-b flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <h2 className="font-heading text-xl font-bold text-primary">Notices</h2>
                    ...
                    <tr key={item.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4">
                        <div className="flex items-center">
                          {item.type === 'urgent' && <AlertTriangle className="h-5 w-5 text-red-500 mr-2" />}
                          <div>
